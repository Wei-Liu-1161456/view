...
class CustomerHome:
    def __init__(self, root, customer):
        ...
        # 初始化product_system为None
        self.product_system = None
    ...
    def update_display_area(self, title, content=""):
        """更新显示区域的内容"""
        # 如果存在product_system，先清理它
        if self.product_system:
            self.product_system.get_main_frame().destroy()
            self.product_system = None
        ...
        # 重新显示界面
        if content:
            ttk.Label(self.display_frame, text=content).pack(pady=10)

    def place_new_order(self):
        """显示订单系统"""
        try:
            # 如果已经存在product_system，先清理它
            if self.product_system:
                self.product_system.get_main_frame().destroy()
            ...
            # 创建新的Product实例并显示
            self.product_system = Product(self.display_frame)
            self.product_system.get_main_frame().pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def on_logout(self):
        ...
        if messagebox.askyesno("Logout Confirmation", "Are you sure you want to logout?"):
            try:
                self.root.destroy()  # 直接销毁整个 CustomerHome 窗口
